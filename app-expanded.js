...
document.documentElement.dataset.appBuild = "expanded-505";
...
async function loadPhotos() {
  ...
  window.KOLLECTION_DEBUG = {
    staticItems: staticItems.length,
    staticImages: staticItems.reduce((sum, item) => sum + (item.images?.length || 0), 0)
  };
  state.photos = normalizePostEntries(staticItems);
  window.KOLLECTION_DEBUG.normalizedPhotos = state.photos.length;
